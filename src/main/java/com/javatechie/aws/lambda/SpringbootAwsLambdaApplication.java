...
public class SpringbootAwsLambdaApplication {
	...
	public Function<APIGatewayProxyRequestEvent, List<Order>> findOrderByName() {
		...
		// order.getName().equals(input)).collect(Collectors.toList());

		return (input) -> orderDao.buildOrders().stream().filter(o -> o.getName().equalsIgnoreCase(input.getQueryStringParameters().get("ordername")))
